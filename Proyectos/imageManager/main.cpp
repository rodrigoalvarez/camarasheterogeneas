...
void loadLightMapTexture(Model_IMG* model, string file) {
    ...
    model->Load(file);
    //cout << model->Width << " - " << model->Height << " - " << model->Pixels[(model->Width * model->Height * 3)-2] << endl;
...
glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, model->Width,model->Height, 0, GL_RGB, GL_UNSIGNED_BYTE, model->Pixels);
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < 1; i++) {//textureCount
        glActiveTextureARB(GL_TEXTURE0 + i);
